...
public class ActivityAdvertising1 extends Activity implements View.OnClickListener, SurfaceHolder.Callback {
    ...
    private void nextPlayer(boolean mark) {
        ...
        if (currentVideoIndex < VideoListQueue.size()) {
            ...
            myHandler.postDelayed(new Runnable(){
                @Override
                public void run(){
                    Intent i = new Intent(ActivityAdvertising1.this, ActivityAdvertising2.class);
                    i.putExtra("index", currentVideoIndex);
                    i.putExtra("datas", (Serializable)VideoListQueue);
                    i.putExtra("mode", mode);
                    ActivityAdvertising1.this.startActivity(i);

                    WindowManager.LayoutParams lp = window.getAttributes();
                    lp.alpha = 0.0f;
                    window.setAttributes(lp);
                    ActivityAdvertising1.this.finish();
                }
            }, 1000);
        }else {
            WindowManager.LayoutParams lp = window.getAttributes();
